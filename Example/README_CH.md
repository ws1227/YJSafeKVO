...
将代码`[foo observeKeyPath:@"name" ...]` 替换为 `[foo observeKeyPath:@keyPath(foo.name) ...]`，`@keyPath`只会裁剪掉字符串中首个点号前面的部分，所以不要把`self.foo.name`那去做静态检查，否则会生成`foo.name`作为方法的key path的参数而导致出错。具体详见`YJKVCMacros.h`。
